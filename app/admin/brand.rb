ActiveAdmin.register Brand do
  index do
    ...
    column :created_at
    column :updated_at
    column :hidden do |brand|
      check_box_tag(nil, nil, brand.hidden?, disabled: true, id: nil)
    end
    default_actions
  end
  ...
  show do |s|
    attributes_table do
      ...
      row :hidden do
        check_box_tag nil, nil, s.hidden?, disabled: true, id: nil
      end
    end
  end
  ...
  controller do
    def permitted_params
      params.permit brand: [:name, :recaptcha_public_key, :logo, :email, :iim, :terms, :hidden]
    end
  end
end
